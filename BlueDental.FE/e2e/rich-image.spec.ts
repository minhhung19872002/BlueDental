...
test.describe("Rich text images", () => {
  ...
  test("Danh mục keeps an embedded picture at full strength", async ({ page }) => {
    ...
    // This editor stores the picture as it is, so it is finished the moment it
    // lands and must not be dimmed as though it were still uploading.
  ...
  test("Vận hành swaps to the stored picture without it vanishing", async ({ page }) => {
    ...
    await expect(dialog.locator('.ql-editor img[src*="/article-images/"]')).toBeVisible();
