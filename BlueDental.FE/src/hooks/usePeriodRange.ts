...
/**
 * The window a report or history list is read through.
 *
 * The reference gives every such screen the same Ngày / Tuần / Tháng (/ Năm)
 * switch with a stepper beside it, so a list always shows one whole period and
 * the arrows walk to the next or previous one. The server squares the anchor
 * date to the period, so all this has to carry is which period and which date.
 */
...
/** The first and last day (inclusive) of the window being read. */
export function periodBounds(period: ReportPeriod, anchor: Date): { start: Date; end: Date } {
  if (period === "day") {
  ...
  if (period === "month") {
    ...
    return { start, end: new Date(anchor.getFullYear(), anchor.getMonth() + 1, 0) };
  }
