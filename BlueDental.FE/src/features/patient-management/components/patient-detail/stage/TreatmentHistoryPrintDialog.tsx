...
function Rows({ stages, statusOf }: Pick<Props, "stages" | "statusOf">) {
  ...
  return (
    <tbody>
      {stages.map((stage) => {
        const status = statusOf(stage.treatmentServiceId);
        return (
          <tr key={stage.id}>
            <td>
            ...
            <td>
              ...
              {status !== null && (
                <div className="pd-print-chip">{lineStatus[status].label}</div>
              )}
            </td>
...
export function TreatmentHistoryPrintDialog({
  ...
  const handlePrint = () => {
    // Only the sheet goes to the printer: the body carries a print rule that
    // hides everything else while this class is on it.
