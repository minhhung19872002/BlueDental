...
function ActorKind({ isSystem }: { isSystem: boolean }) {
  const text = isSystem ? t("Appointment:History:TypeSystem") : t("Appointment:History:TypeUser");
...
export function HistoryDetailPanel({ entry }: { entry: HistoryEntry }) {
  ...
  const meta: MetaItem[] = [
    { label: t("Appointment:History:AppointmentId"), value: entry.appointmentId, code: true },
    {
      label: t("Common:Status"),
      value: `${statusLabel(entry.statusBefore)} → ${statusLabel(entry.statusAfter)}`,
    },
    { label: t("Appointment:History:OperatorField"), value: entry.actorName },
    { label: "IP", value: dash(entry.ipAddress) },
    { label: t("Appointment:History:Browser"), value: dash(entry.browser) },
    { label: t("Appointment:History:OS"), value: dash(entry.operatingSystem) },
  ];
    ...
    <div className="ah-detail" data-testid="ah-detail">
      <div className="ah-detail-head">
        <div className="ah-detail-badges">
          ...
          <HistoryPill tone="blue">{t("Common:Appointment")}</HistoryPill>
        </div>
      ...
      <div className="ah-detail-grid">
        <section className="ah-card">
          <h4 className="ah-card-title">{t("Appointment:History:Actor")}</h4>
        ...
        <section className="ah-card">
          <h4 className="ah-card-title">{t("Appointment:History:AccessInfo")}</h4>
          <ul className="ah-access">
            ...
            <li>
              ...
              <span className="ah-access-label">{t("Appointment:History:Device")}:</span>
              <span className="ah-access-value">{describeDevice(entry)}</span>
            </li>
            <li>
              <Server size={14} strokeWidth={1.75} />
              <span className="ah-access-label">{t("Appointment:History:Source")}:</span>
