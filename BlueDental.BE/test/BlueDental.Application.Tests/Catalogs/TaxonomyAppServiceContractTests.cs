...
public class TaxonomyAppServiceContractTests
{
    private readonly Type _serviceType = typeof(TaxonomyAppService);
    private readonly Type _interfaceType = typeof(ITaxonomyAppService);

    [Fact]
    public void TaxonomyAppService_Should_Implement_ITaxonomyAppService()
    {
        _interfaceType.IsAssignableFrom(_serviceType).ShouldBeTrue();
    }

    [Fact]
    public void TaxonomyAppService_Should_Inherit_ApplicationService()
    {
        typeof(ApplicationService).IsAssignableFrom(_serviceType).ShouldBeTrue();
    }

    [Fact]
    public void TaxonomyAppService_Should_Require_Authorization_At_Class_Level()
    {
