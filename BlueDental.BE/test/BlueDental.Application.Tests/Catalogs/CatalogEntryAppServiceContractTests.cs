...
public class CatalogEntryAppServiceContractTests
{
    private readonly Type _serviceType = typeof(CatalogEntryAppService);
    private readonly Type _interfaceType = typeof(ICatalogEntryAppService);

    [Fact]
    public void CatalogEntryAppService_Should_Implement_ICatalogEntryAppService()
    {
        _interfaceType.IsAssignableFrom(_serviceType).ShouldBeTrue();
    }

    [Fact]
    public void CatalogEntryAppService_Should_Inherit_ApplicationService()
    {
        typeof(ApplicationService).IsAssignableFrom(_serviceType).ShouldBeTrue();
    }

    [Fact]
    public void CatalogEntryAppService_Should_Require_Authorization_At_Class_Level()
    {
