...
| F-35 | Vận hành — phân loại và bài viết theo khối | `VERIFIED` | `e2e/operations.spec.ts` | Added 2026-08-25. Tám khối là tám route riêng, sub-tab nằm trong query nên link thẳng tới được; phân loại tạo/sửa/xoá, bài viết có tiêu đề và nội dung rich-text, lưu xong reload vẫn còn; "Tạo Bài Viết" khoá lại khi chưa chọn phân loại, đúng như bản gốc. BE thêm `UpdateCategoryAsync` và phân trang thật cho danh sách bài viết. Bổ sung 2026-08-25 (R-93…R-99): dữ liệu **theo chi nhánh** (khác bản gốc — bản gốc trả `branchId: null`, xem `docs/clone/pages/operations.md`), mục mới lên đầu danh sách, hai lệnh sửa/xoá nằm thẳng trên hàng, tìm kiếm cắt khoảng trắng và bỏ qua hoa thường, ảnh trong bài viết lưu ra blob storage và nội dung chỉ giữ link — ảnh hiện ngay lúc chọn rồi mới đổi sang link đã lưu, và `PUT categories/{id}` (thiếu route nên đổi tên trả 405). Năm sub-tab còn lại vẫn chưa quan sát được — xem `docs/clone/unknowns.md` |
