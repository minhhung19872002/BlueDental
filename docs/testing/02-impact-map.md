...
| `TreatmentPlan.ApplyVoucher` | 2 | F-09 and F-11. `VoucherDiscountAmount` was mapped long before anything set it; `PlanDiscountAmount` adds it to the slip discount and caps the sum at the slip total, so touching either changes a slip's total |
| `hooks/useAdviseQuotes.ts` | 2 | F-09 only. Server-backed since 2026-09-09; holds the báo giá tabs. Every setter must be called from the handler, never from inside another updater: StrictMode double-invokes updaters, and an insert in there produced two quotes sharing one id (R-322). Test this on the **dev server** as well as the production build — StrictMode only double-invokes in development, so the production build cannot catch that class of bug |
